...
var soundManager = require("../../../../public/sounds/sounds.js");
...
var GameField = React.createClass({
  ...
  componentDidMount: function() {
    ...
    socket.on("opponent status", function(data) {
      ...
      data.opponentOffline
        ? self.setState({ connectionText: "Соперник не в сети" })
        : self.setState({ connectionText: "" });
    });
    ...
    socket.once("end game", function(data) {
      ...
      switch (data) {
        ...
        case "win":
          ...
          self.setState({
            statusText: "Игра закончилась. Вы выиграли!! УРАА!"
          });
  ...
  addGameStatusListener: function() {
    ...
    socket.on("game status", function(gameData) {
      if (self.gameStatusListen) {
        ...
        frameColorClassNew = "";
        gameData.nowTurn
          ? (frameColorClassNew = "framecolormyturn")
          : (frameColorClassNew = "framecolornotmyturn");
  ...
  isZoneUnderAttack: function(fieldState, zone, myNumber) {
    ...
    if (myNumber == 1) {
      ...
      if (hypoteticPawnCoords.i >= 0 && hypoteticPawnCoords.j <= 7) {
        if (
          fieldState[hypoteticPawnCoords.i][hypoteticPawnCoords.j] == "pawn_b"
        )
          return true;
      }
    ...
    if (myNumber == 2) {
      ...
      if (hypoteticPawnCoords.i >= 0 && hypoteticPawnCoords.j <= 7) {
        if (
          fieldState[hypoteticPawnCoords.i][hypoteticPawnCoords.j] == "pawn_w"
        )
          return true;
      }
    ...
    for (var i = zone.i - 1; i >= 0; i--) {
      if (
        fieldState[i][zone.j] != "empty" &&
        fieldState[i][zone.j] != "rook" + ending &&
        fieldState[i][zone.j] != "queen" + ending
      ) {
        //если другая фигура
        break;
      }
    ...
    for (var j = zone.j - 1; j >= 0; j--) {
      if (
        fieldState[zone.i][j] != "empty" &&
        fieldState[zone.i][j] != "rook" + ending &&
        fieldState[zone.i][j] != "queen" + ending
      ) {
        //если другая фигура
        break;
      }
    ...
    for (var i = zone.i - 1, j = zone.j + 1; i >= 0 && j <= 7; i--, j++) {
      if (
        fieldState[i][j] != "empty" &&
        fieldState[i][j] != "bishop" + ending &&
        fieldState[i][j] != "queen" + ending
      ) {
        //если другая фигура
        break;
      }
    ...
    for (var i = 0; i < hypoteticKnightCoords.length; i++) {
      if (
        hypoteticKnightCoords[i].i <= 7 &&
        hypoteticKnightCoords[i].i >= 0 &&
        hypoteticKnightCoords[i].j <= 7 &&
        hypoteticKnightCoords[i].j >= 0
      ) {
        if (
          fieldState[hypoteticKnightCoords[i].i][hypoteticKnightCoords[i].j] ==
          "knight" + ending
        )
          return true;
      }
    ...
    for (var i = 0; i < hypoteticKingCoords.length; i++) {
      if (
        hypoteticKingCoords[i].i <= 7 &&
        hypoteticKingCoords[i].i >= 0 &&
        hypoteticKingCoords[i].j <= 7 &&
        hypoteticKingCoords[i].j >= 0
      ) {
        if (
          fieldState[hypoteticKingCoords[i].i][hypoteticKingCoords[i].j] ==
          "king" + ending
        )
          return true;
      }
  ...
  isTurnPossible: function(selectedFigure, placeToMove) {
    ...
    switch (selectedFigure.class) {
      case "pawn_w":
        if (
          selectedFigure.i == placeToMove.i &&
          selectedFigure.j == placeToMove.j - 1 &&
          placeToMove.class == "empty"
        ) {
          result = true;
          //в дамки
        ...
        if (
          selectedFigure.i == placeToMove.i &&
          selectedFigure.j == placeToMove.j - 2 &&
          placeToMove.class == "empty" &&
          selectedFigure.j == 1
        ) {
          result = true;
        ...
        //поедание
        if (
          (selectedFigure.i == placeToMove.i + 1 ||
            selectedFigure.i == placeToMove.i - 1) &&
          selectedFigure.j == placeToMove.j - 1 &&
          opponentAttacked()
        ) {
      ...
      case "bishop_w":
        ...
        var f = true;
        if (
          Math.abs(placeToMove.i - selectedFigure.i) ==
          Math.abs(placeToMove.j - selectedFigure.j)
        ) {
          var i = selectedFigure.i;
          var j = selectedFigure.j;
          var di =
            (placeToMove.i - selectedFigure.i) /
            Math.abs(selectedFigure.i - placeToMove.i);
          var dj =
            (placeToMove.j - selectedFigure.j) /
            Math.abs(selectedFigure.j - placeToMove.j);
          //alert("i j di dj" + i + " "+ j + " "+ di + " "+ dj + " ");
          for (
            var a = 0;
            a < Math.abs(selectedFigure.i - placeToMove.i) - 1;
            a++
          ) {
            i = i + di;
            j = j + dj;
      ...
      case "king_w":
        if (
          Math.abs(placeToMove.i - selectedFigure.i) <= 1 &&
          Math.abs(placeToMove.j - selectedFigure.j) <= 1
        ) {
          if (
            placeToMove.i == selectedFigure.i &&
            placeToMove.j == selectedFigure.j
        ...
        var f = true;
        if (
          Math.abs(placeToMove.i - selectedFigure.i) == 2 &&
          Math.abs(placeToMove.j - selectedFigure.j) == 0
        ) {
          if (
            placeToMove.i == 6 &&
            placeToMove.j == 0 &&
            !this.state.moved.king_w &&
            !this.state.moved.rook_w_r
          ) {
            for (var i = 5; i < 7; i++) {
          ...
          if (
            placeToMove.i == 6 &&
            placeToMove.j == 7 &&
            !this.state.moved.king_b &&
            !this.state.moved.rook_b_r
          ) {
            for (var i = 5; i < 7; i++) {
  ...
  clickHandler: function(e) {
    ...
    if (this.state.myTurn) {
      if (!this.state.selectedFigure.selected) {
        ...
        var turnPossibleResult = this.isTurnPossible(
        ...
        if (turnPossibleResult) {
          ...
          tmpFieldState[this.state.selectedFigure.i][
            this.state.selectedFigure.j
          ] = "empty";
          ...
          if (turnPossibleResult == "queen_w") {
            ...
            tmpFieldState[placeToMove.i][
              placeToMove.j
            ] = this.state.selectedFigure.class;
          }
          ...
          if (!this.isCheck(tmpFieldState)) {
            ...
            socket.emit("turn done", {
              playerNumber: this.state.myNumber,
              field: tmpFieldState,
              moved: this.state.moved,
              turnContent: turnContent,
              lostFigure: lostFigure
            });
            ...
            var func = function() {
              ...
              self.setState({
                frameColorClass: self.state.myTurn
                  ? self.timeoutData.frameColorClass
                  : self.state.frameColorClass
              });
            };
          ...
          //Снять выделение
          if (
            this.state.selectedFigure.i == placeToMove.i &&
            this.state.selectedFigure.j == placeToMove.j
          ) {
            this.setState({
              statusText: "Ваш ход!",
              selectedFigure: {
    ...
    function htmlField() {
      ...
      function horizontalFrame() {
        for (var i = 0; i < self.state.fieldState[0].length; i++) {
          result.push(
            <div
              id={framecnt}
              className={"horizontframe " + self.state.frameColorClass}
              key={framecnt}
            >
      ...
      function verticalFrame(j) {
        result.push(
          <div
            id={framecnt}
            className={"verticalframe " + self.state.frameColorClass}
            key={framecnt}
          >
            {j}
          </div>
      ...
      function cornerFrame() {
        result.push(
          <div
            id={framecnt}
            className={"cornerframe " + self.state.frameColorClass}
            key={framecnt}
          />
        );
        framecnt++;
      ...
      for (var j = self.state.fieldState.length - 1; j > -1; j--) {
        ...
        for (var i = 0; i < self.state.fieldState[j].length; i++) {
          if (self.state.selectedFigure.selected) {
            if (
              self.state.selectedFigure.i == i &&
              self.state.selectedFigure.j == j
            ) {
              selected = "selected";
            }
          }
          if (self.state.lastOpponentTurn) {
            if (
              (self.state.lastOpponentTurn[0][0] == i &&
                self.state.lastOpponentTurn[0][1] == j) ||
              (self.state.lastOpponentTurn[1][0] == i &&
                self.state.lastOpponentTurn[1][1] == j)
            ) {
        ...
        <div>
          ...
          <div id="statusbar">
            <StatusBar
              ...
              connectionText={this.state.connectionText}
              button1={this.state.statusButton1}
              button2={this.state.statusButton2}
              buttonsDescriptionText={this.state.buttonsDescriptionText}
            />
            <OpponentName />
          </div>
        </div>
